      ...
      .then(async res => {
        ...
        fetch(`${process.env.NEXT_PUBLIC_API}${_uri}`, {
          ...
          .then(res => {
            ...
            let _merchantNames = []
            res?.data?.map(item => {
              if (!includes(_merchantNames, item.merchant_name)) {
                _merchantNames.push(item.merchant_name)
              }
            })
            setMerchantNames(_merchantNames)
            setData(res?.data)
            getUserPrivileges()
            setLoading(false)
          })
      ...
      .then(async res => {
        const _uri =
          isAdd === true && isDelete === false
            ? '/admin/master/user/creates'
            : isDelete === true
            ? '/admin/master/user/deletes'
            : '/admin/master/user/updates'
        const _secret = await generateSignature(_uri)
        ...
        // console.log('_valueModal: ', _valueModal)
        // setLoading(false)
  ...
  return (
    <Grid container spacing={6}>
      <Grid item xs={12}>
        ...
        <Typography variant='body2'>Semua User Merchant yang tersedia</Typography>
        <Divider />
      ...
      >
        ...
        <Box sx={{ p: 2 }}>
          ...
          <Select
            ...
            label='Privilege'
            size='small'
            error={errorsField?.user_privilege}
            helperText={errorsField?.user_privilege}
            onChange={e => handleChangeEl('user_privilege', e, valueModal, setValueModal, schemaData, setErrorsField)}
          >
            {filter(userPrivileges, ['user_privilege_type', '2'])?.map(item => (
