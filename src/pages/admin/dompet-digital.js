...
const MUITable = () => {
  ...
  const getData = async (
    ...
    fetch(`${process.env.NEXT_PUBLIC_API}/auth/check_auth`, {
      ...
      .then(async res => {
        const _uri = '/admin/transactions/journal/list_ewallet'
        const _secret = await generateSignature(_uri)
  ...
  const handleCheckDeposit = async (_amount, _inv_no, _pg_method) => {
    ...
    // console.log('_pg_method: ', _pg_method)
    // console.log('paymentMethods: ', _pg_method)
  ...
  return (
    <Grid container spacing={6}>
      <Grid item xs={12}>
        ...
        <Typography variant='body2'>History Dompet Digital Merchant</Typography>
      </Grid>
              ...
              <>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center', mb: 7 }}>
                  {parseInt(valuePM?.id) == 18 || parseInt(valuePM?.id) == 19 ? (
                    <>
                      <img src={`/${paymentDetail?.image_src}`} />
                    </>
